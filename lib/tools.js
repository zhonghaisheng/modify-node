...
module.exports = {
    ...
    footTextNone: function(footText, outLabel, html, myfooterText, htmlIndex) {
        ...
        if (footText == '') {
            var footerTextReg = outLabelBefore + "(.*)<\/" + outLabelAfter2;
        } else {
            var footerTextReg = outLabelBefore + footText+"<\/" + outLabelAfter2;
        }

        if (Array.isArray(myfooterText) && (typeof myfooterText[htmlIndex] != 'undefined')) {
            html = html.replaceAll(footerTextReg, outLabelBefore + myfooterText[htmlIndex] + outLabelAfter);
        } else if (typeof myfooterText == 'string') {
            html = html.replaceAll(footerTextReg, outLabelBefore + myfooterText + outLabelAfter);
        } else if (Array.isArray(myfooterText) && (typeof myfooterText[htmlIndex] == 'undefined')) {
            html = html.replaceAll(footerTextReg, outLabelBefore2 + " style='display:none'> " + outLabelAfter);
        }
        return html;
    },
